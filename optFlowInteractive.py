...
step = 16           # step size for vectors. Larger value = less vectors displayed
...
imagePath = "/Users/bementmbp/Desktop/circleTest3.tif"    #path to 1-channel image
imageStack=skio.imread(imagePath)           # reads image as ndArray
firstFrame = imageStack[0]                  # sets first image frame
secondFrame = imageStack[0+framesToSkip+1]  # sets second image frame
...
fig.subplots_adjust(top = 0.9)                      # sets spacing for top subplot
...
n, bins = np.histogram(x, bins=numBins)         # n is the counts and bins is ndarray of the equally spaced bins between (-pi, pi)
widths = np.diff(bins)                          # ndarray; width of each bin
area = n / x.size                               # fraction of the total counts in each bin; area to assign each bin
...
def update(val):
    ...
    newAngles = newAngles[newAngles != 0.]
    ...
    newN, newBins = np.histogram(newX, bins=numBins)                # Bin data and record counts
