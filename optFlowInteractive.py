...
frames = list(Path('/Users/bementmbp/Desktop/bzOptFlow/imageSequence').iterdir())
frames.sort()
step = 5
framesToCompare = 1
...
arrows = ax[1].quiver(np.arange(0, vectors.shape[1], step), np.flipud(np.arange(vectors.shape[0], -1, -step)), 
...
try:
    winSlider = Slider(ax=winAx, label='win', valmin=5, valmax=50, valinit=20, valfmt=' %0.0f Px', valstep=winValues, facecolor='#cc7000')
except ValueError:
